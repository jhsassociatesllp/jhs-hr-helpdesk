...
async function loadTickets() {
    ...
    try {
        ...
        // Newest tickets on top, oldest at the bottom
...
function renderTickets() {
    ...
    tbody.innerHTML = paged.map(t => {
        ...
        // Full description preserved for hover tooltip; only the display text is shortened
        const fullDescription = t.issueDescription || '';
        ...
        return `<tr>
            ...
            <td>${escapeHtml(t.assetCode) || '-'}</td>
            <td>${escapeHtml(issuesText)}</td>
            <td class="truncate-cell" data-full="${escapeHtml(fullDescription)}">${escapeHtml(shortDescription)}</td>
            <td>${escapeHtml(t.reportingPartner) || '-'}</td>
            <td>${statusBadge}</td>
            <td>${fmtDateIsoToIST(t.createdAt)}</td>
            <td>${escapeHtml(t.assigned) || 'Unassigned'}</td>
            ...
            <td>${fmtDateIsoToIST(t.assignedAt)}</td>
            <td>${fmtDateIsoToIST(t.closedAt)}</td>
            <td class="truncate-cell" data-full="${escapeHtml(t.remark || '')}">${escapeHtml((t.remark || '').slice(0, 50))}${t.remark && t.remark.length > 50 ? '...' : ''}</td>
            <td><strong>${escapeHtml(tat)}</strong></td>
            <td><button class="small" onclick="deleteTicket('${escapeHtml(t.id)}')">Delete</button></td>
        </tr>`;
    }).join('');
...
// CSS ::after based tooltips get clipped by .table-wrap's overflow-x:auto
// (setting one overflow axis forces the other to clip too), so we render
// the tooltip as a floating element attached to <body> instead.
(function setupCellTooltip() {
    ...
    function positionTooltip(el, mouseX, mouseY) {
        ...
        const maxTop = window.innerHeight - tip.offsetHeight - 10;
        if (left > maxLeft) left = Math.max(10, mouseX - tip.offsetWidth - margin);
        if (top > maxTop) top = Math.max(10, mouseY - tip.offsetHeight - margin);
    ...
    document.addEventListener('mouseover', (e) => {
        ...
        positionTooltip(cell, e.clientX, e.clientY);
    });

    document.addEventListener('mousemove', (e) => {
        const cell = e.target.closest('.truncate-cell');
        if (!cell || !tooltipEl || tooltipEl.style.display === 'none') return;
        positionTooltip(cell, e.clientX, e.clientY);
    });
    ...
    document.addEventListener('mouseout', (e) => {
        ...
        if (toEl && cell.contains(toEl)) return;
